...
def draw_box_label_on_image(img,obj_boxes_list, box_color=(0, 255, 255), show_label=True):
    '''
    obj_boxes_list is the list of all boxes in img
    Helper funciton for drawing the bounding boxes and the labels
    bbox_cv2 = [left, top, right, bottom]
    '''
    #box_color= (0, 255, 255)
    for bbox_cv2 in obj_boxes_list:
        font = cv2.FONT_HERSHEY_SIMPLEX
        font_size = 0.7
        font_color = (0, 0, 0)
        left, top, right, bottom = bbox_cv2[1], bbox_cv2[0], bbox_cv2[3], bbox_cv2[2]
